...
def make_weird_sentence(line, words_from_line, weird_words):
    ...
    for i in range(len(words_from_line)):
        weird_sentence = weird_sentence.replace(
            words_from_line[i], weird_words[i])
...
def make_weird_words(words_from_line):
    ...
    for i in range(len(words_from_line)):
        if len(words_from_line[i]) <= 3:
            ...
            weird_word = words_from_line[i][0] + "".join(
                permuted_letters) + words_from_line[i][len(words_from_line[i]) - 1]
    ...
    try:
        with open(sys.argv[1], "r") as input_file:
            ...
            for line in input_file:
                words_from_line = re.findall(
                    r'(\w+)', line.replace(r'\n', ''), re.U)
                words_from_input_file.extend(words_from_line)

                weird_words = make_weird_words(words_from_line)
                weird_sentence = make_weird_sentence(
                    line, words_from_line, weird_words)
                output_file.writelines(weird_sentence)

            output_file.writelines("\n---weird---\n")
            output_file.writelines(
                " ".join(sorted(words_from_input_file, key=str.lower)))
            output_file.close()
